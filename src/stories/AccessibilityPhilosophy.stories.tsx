...
/**
 * Accessibility is product quality, not a compliance checkbox.
 *
 * Every component on the site meets WCAG 2.1 AA and Section 508:
 * keyboard-first paths, visible `focus-visible` rings, semantic HTML,
 * screen-reader parity, reduced-motion parity, and color contrast that
 * survives real lighting.
 */
const meta = {
  title: "Philosophy/Accessibility as Product Quality",
  parameters: {
    layout: "padded",
    docs: {
      description: {
        component:
          "Accessibility investment compounds — it improves test coverage, sharpens component APIs, and speeds onboarding. Every primitive on this site is keyboard-navigable, motion-safe, and screen-reader parity is verified.",
      },
    },
  },
} satisfies Meta;
...
const items: Array<[string, string]> = [
  ...
  ["Color contrast", "AA minimum, AAA where it counts."],
];
...
export const Checklist: Story = {
  render: () => (
    <ul className="grid max-w-3xl gap-3 md:grid-cols-2">
      {items.map(([t, d]) => (
        <li key={t} className="edge-gold border border-border bg-card p-4">
...
export const FocusStates: Story = {
  render: () => (
    <div className="flex flex-wrap items-center gap-4">
      ...
      <a
        href="#"
        className="story-link text-[14px] text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-gold focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      >
        Focused link
      </a>
    </div>
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Tab through the demo. Every focusable element has a visible ring in the gold token, offset from the background for contrast.",
      },
      source: {
